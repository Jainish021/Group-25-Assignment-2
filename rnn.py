...
class RNN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        hidden = torch.zeros(self.numOfLayer, inputs.size(
            1), self.h).requires_grad_()

        # [to fill] obtain output layer representations
        output_layer, hidden = self.rnn(inputs, hidden.detach())

        # [to fill] sum over output
        Sum = self.W(torch.sum(output_layer, dim=0))
...
if __name__ == "__main__":
    ...
    word_embedding = pickle.load(
        open('./Data_Embedding/word_embedding.pkl', 'rb'))
    ...
    test_output = np.empty(len(test))
    for i in range(len(test)):
        input_words, gold_label = test[i]
        input_words = " ".join(input_words)
        ...
        # Transform the input into required shape
        # vectors = torch.tensor(vectors).view(len(vectors), 1, -1)
        vectors = torch.tensor(np.concatenate(
            vectors, axis=0)).view(len(vectors), 1, -1)
        output = model(vectors)
        predicted_label = torch.argmax(output)
    ...
    string = './results/RNN - HiddenLayerDim - ' + \
